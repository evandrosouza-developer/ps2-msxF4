...
void tim2_isr(void)
{
	...
	if (timer_get_flag(TIM2, TIM_SR_UIF))
	{
		...
		gpio_clear(TIM2UIF_PORT, TIM2UIF_PIN_ID); //Signs start of interruption
		...
		//Debug & performance measurement
		gpio_set(TIM2UIF_PORT, TIM2UIF_PIN_ID); //Signs end of interruption
	}	//if (timer_get_flag(TIM2, TIM_SR_UIF))
	else if (timer_get_flag(TIM2, TIM_SR_CC1IF))
	{
		...
		//Debug & performance measurement
		gpio_clear(TIM2CC1_PORT, TIM2CC1_PIN_ID); //Signs start of interruption
    ...
    //timer_clear_flag(TIM2, TIM_SR_CC1OF |  TIM_SR_CC1IF);
		...
		//Debug & performance measurement
		gpio_set(TIM2CC1_PORT, TIM2CC1_PIN_ID); //Signs end of TIM2 interruption. Default condition is "1"
	}
}
